...
<body >
	...
	<div id="container">
		...
		<h2 id="listTit">您的位置>統計報表><span>季報表</span></h2>

	<div id="listCon">
            <div class="temCon">
                <form:form id="myform" method="POST"
                    commandName="voQuarterlyReportCondition">
                    <table width="540">
                        <tr>
                            <td>自:</td>
                            ...
                            <td>Q<form:select path="fromQuarter" class="selm">
                                    <form:options items="${quarters}" />
                                </form:select></td>
                            <td>至:</td>
                            <td><form:select path="toYear" class="sely">
                                    <form:options items="${years}" />
                                </form:select></td>
                            <td>Q<form:select path="toQuarter" class="selm">
                                    <form:options items="${quarters}" />
                                </form:select></td>
		...
		<a href="ExcelController.htm?output=ExcelQuarterlyReportView" class="exportBtn">輸 出 Excel 表</a>

		<!-- xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx -->

		<table width="1228" height="122" border="1" cellpadding="0" cellspacing="0" class="temTable">
			<tr>
				<th rowspan="2" valign="middle">季</th>
			...
			<c:forEach items="${quarterlyreportlist}" var="mr">
				...
				<tr>
					<td><c:choose>
							...
							<c:when test="${mr.value.status}">
								<a href="theQuarterReports.htm?key=${mr.key}&output=view">${mr.key}</a>
							</c:when>
							<c:otherwise>

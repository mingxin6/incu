 ...
 --%>
			...
			<div id="listCon">
				...
				<c:if test="${!user.isBusiness}">
					...
					<a href="compList_B.htm" class="formIcon"> <s:message
							code="button.backlist" text="回列表" /> </a>


					<c:if
						test="${comp.isNewApplying ||comp.isAccepted || comp.isAboutToReview}">
						<a href="compDetailEdit_B.htm?compID=${comp.uid}" class="formIcon"><s:message
								code="compdetail.modify_upload" text="修改及資料上傳" />
						</a>
					</c:if>
					...
					<c:if test="${comp.isNewApplying}">
						<a href="compAccept_B.htm?compID=${comp.uid}" class="formIcon">資料備齊</a>
						<a href="compDeny_B.htm?compID=${comp.uid}" class="formIcon">中止申請</a>
					</c:if>
				...
				<div class="header">
					<s:message code="" text="" />
					<s:message code="compdetail.compstatus" text="企業狀態" />
					<h2 style="color: red">
						(${comp.compStatusRS})
					</h2>
					<br />

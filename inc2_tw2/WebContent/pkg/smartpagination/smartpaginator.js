	...
	$.fn.extend({
		smartpaginator : function(options) {
			...
			return this.each(function() {
				...
				if (settings.totalrecords % settings.recordsperpage > 0)
					totalpages++;
				...
				function showLabels(pageIndex) {
					...
					container.append($('<span/>').append($('<b/>').text(pageIndex * settings.recordsperpage + 1))).append($('<span/>').text('-')).append($('<span/>').append($('<b/>').text(upper))).append($('<span/>').text('of')).append($('<span/>').append($('<b/>').text(settings.totalrecords)));
				}
				function buildNavigation(startPage) {
					...
					for (var i = startPage; i < startPage + settings.length; i++) {
						if (i == totalpages)
							break;
						list.append($('<li/>').append($('<a>').attr('id', (i + 1)).addClass(settings.theme).addClass('normal').attr('href', 'javascript:void(0)').text(i + 1)).click(function() {
					...
					if (topage >= 0 && topage < totalpages) {
						...
						if (dataContainer != null) {
							if (dataContainer.length > 0) {
								...
								if ($(dataElements[0]).find('th').length > 0) { // if
									// there
									// is a
									// header,
									// keep

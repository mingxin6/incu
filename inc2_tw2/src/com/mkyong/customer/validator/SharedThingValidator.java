...
public class SharedThingValidator implements Validator {

	private static transient Logger logger = LoggerFactory.getLogger(SharedThingValidator.class);

	@Override
	public boolean supports(Class clazz) {
		return SharedThing.class.isAssignableFrom(clazz);
	}

	@Override
	public void validate(Object target, Errors errors) {
		ValidationUtils.rejectIfEmptyOrWhitespace(errors, "title", "标题不可空白.", "标题不可空白.");

		SharedThing obj = (SharedThing) target;

		List<MultipartFile> files = obj.getFiles();
		if (files == null)
			return;
